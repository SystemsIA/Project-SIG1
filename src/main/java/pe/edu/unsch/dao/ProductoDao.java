...
public interface ProductoDao extends JpaRepository<Producto, Serializable> {
	...
	@Query("select new Producto(pro.codigo as codigo, pro.nombre as nombre, "
			+ "pro.descripcion as descripcion, pro.cantidad as cantidad, "
			+ "pro.precioUnitario as precioUnitario, "
			+ " cat.nombre as marca, pro.persona as persona) "
			+ "from Producto pro "
			+ "inner join pro.categoriaProductos cp "
			+ "inner join cp.categoria cat "
			+ "inner join pro.persona per "
